...
public class Service {
    ...
    public void login(Scanner sc) {
		...
		*/
		...
		Login result = this.loginDAO.login(id_, pw_);
		if (result == null) {
			System.out.println("아이디 또는 패스워드가 틀렸습니다.");
		} else {
			System.out.printf("관리자 '%s' 로그인되었습니다.%n", result.getId_());
			this.id_ = result.getId_();
			this.main(sc);
		}
	...
	private void menu1_sub1(Scanner sc) {
		...
		int result = this.employeeDAO.add(new Employee(null, name, ssn, Date.valueOf(hiredate), phone, null, null, null, regId, deptId, jobId,
				basicpay, extrapay, (basicpay + extrapay)));
		if (result > 0) {
	...
	private void menu1_sub3_sub1(Scanner sc) {
		...
		List<Employee> list = this.employeeDAO.list("empId", empId);
		if (list.size() == 0) {
	...
	private void menu1_sub3_sub2(Scanner sc) {
		...
		List<Employee> list = this.employeeDAO.list("name_", name_);
		if (list.size() == 0) {
			System.out.println("검색 결과가 없습니다.");
		} else {
			this.employeePrint(list);
	...
	private void menu1_sub3_sub3(Scanner sc) {
		...
		List<Employee> list = this.employeeDAO.list("reg_name", reg_name);
		if (list.size() == 0) {
			System.out.println("검색 결과가 없습니다.");
		} else {
			this.employeePrint(list);
	...
	private void menu1_sub3_sub4(Scanner sc) {
		...
		List<Employee> list = this.employeeDAO.list("dept_name", dept_name);
		if (list.size() == 0) {
			System.out.println("검색 결과가 없습니다.");
		} else {
			this.employeePrint(list);
	...
	private void menu1_sub3_sub5(Scanner sc) {
		...
		List<Employee> list = this.employeeDAO.list("job_title", job_title);
		if (list.size() == 0) {
			System.out.println("검색 결과가 없습니다.");
		} else {
			this.employeePrint(list);
	...
	private void menu1_sub4(Scanner sc) {
		...
		if (list.size() == 0) {
			...
			this.employeePrint(list);
			System.out.print("현재 직원 정보를 삭제할까요(0/1)>");
			int m = sc.nextInt();
			sc.nextLine();
			if (m == 1) {
	...
	private void menu2(Scanner sc) {
		...
		*/
		while (true) {
			...
			switch (m) {
			case 1:	this.menu2_sub1(sc); break;
			case 2:	this.menu2_sub2(sc); break;
			case 3:	this.menu2_sub3(sc); break;
			}
		}
	}
	...
	private void menu2_sub1(Scanner sc) {
		...
		*/
		while (true) {
			...
			switch (m) {
			case 1: this.menu2_sub1_sub1(sc); break;
			case 2: this.menu2_sub1_sub2(); break;
			case 3: this.menu2_sub1_sub3(sc); break;
			}
		}
    ...
    private void menu2_sub2(Scanner sc) {
        while (true) {
            ...
            switch (m) {
            case 1: this.menu2_sub2_sub1(sc); break;
            case 2: this.menu2_sub2_sub2(); break;
            case 3: this.menu2_sub2_sub3(sc); break;
            }
        }
    }
    ...
    private void menu2_sub2_sub3(Scanner sc) {
        ...
        int result = this.departmentDAO.remove(new Department(deptId, null));
        if (result == 1) {
            System.out.printf("%s 부서 삭제 성공!%n", deptId);
        } else {
            System.out.printf("%s 부서 삭제 실패!%n", deptId);
        }
    ...
    private void menu2_sub3(Scanner sc) {
        while (true) {
            ...
            switch (m) {
            case 1: this.menu2_sub3_sub1(sc); break;
            case 2: this.menu2_sub3_sub2(); break;
            case 3: this.menu2_sub3_sub3(sc); break;
            }
        }
    ...
    private void menu2_sub3_sub3(Scanner sc) {
        ...
        System.out.println("-----------------");
        System.out.println("직위번호 / 직위명 / 기본급 / 삭제가능여부");
        for (Job j : jobs) {
        ...
        if (result == 1) {
            System.out.printf("%s 직위 삭제 성공!%n", jobId);
        } else {
            System.out.printf("%s 직위 삭제 실패!%n", jobId);
        }
	...
	private void menu3(Scanner sc) {
		...
		*/
		...
		while (true) {
			...
			switch (m) {
			case 1: this.menu3_sub1(sc); break;
			}
		}
	...
	private void menu3_sub1(Scanner sc) {
		...
		*/
		...
		int result = this.loginDAO.changePW(this.id_, oldPW, newPW);
		if (result > 0) {
			System.out.printf("관리자 '%s'의 패스워드가 변경되었습니다.", this.id_);
		} else {
