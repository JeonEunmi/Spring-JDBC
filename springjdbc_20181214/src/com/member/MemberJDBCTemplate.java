...
public class MemberJDBCTemplate {
	...
	public List<Member> memberSearch(String key, String value) {
		String sql = "SELECT mid_, name_,phone, email, regDate FROM members ";
	...
	public int add(Member m) {
		...
		String sql = "INSERT INTO members(mid_, name_, phone, email, regDate) VALUES"
				+ " ((SELECT CONCAT('M', LPAD(IFNULL(SUBSTR(MAX(mid_), 2), 0)"
				+ " + 1, 2, 0)) AS newId FROM members m), ?, ?, ?, now())";

		int result = this.jdbcTemplateObject.update(sql, m.getName_(), m.getPhone(), m.getEmail());
